...
import { useVibeStore, selectFilteredEvents, selectCurrentCard, selectSavedEventObjects } from '../store/useVibeStore';
...
describe('useVibeStore — user slice', () => {
  ...
  it('saveEvent does not add duplicates', () => {
    ...
    expect(result.current.savedEvents.filter(id => id === 'evt-001')).toHaveLength(1);
  });
...
describe('store selectors', () => {
  ...
  it('selectSavedEventObjects returns matching event objects', () => {
    ...
    expect(saved.map(e => e.id)).toEqual(expect.arrayContaining(['evt-001', 'evt-003']));
